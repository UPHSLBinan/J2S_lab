...
<div class="container">
	...
	if(empty($firstname) or empty($lastname) or empty($age) or empty($middlename) or empty($birthday) or empty($address) or empty($username)or empty($pass)or empty($pass2))
	{
	...
	if (count($errors) > 0) {
		...
		$sql = "INSERT INTO users (username,password,firstname,middlename,lastname,age,birthday,address)
			VALUES(?,?,?,?,?,?,?,?)";
		$stmt = mysqli_stmt_init($conn);
		$preparedStmt = mysqli_stmt_prepare($stmt,$sql);
		if ($preparedStmt){
			mysqli_stmt_bind_param($stmt,"sssssiss",$username,$pass,$firstname,$middlename,$lastname,$age,$birthday,$address);
		...
		<div class="row">
			...
			<div class="col-md-4">
				<div class="mb-3">
					...
					<input type="text" class="form-control" placeholder="Middle Name" name="middlename" id="middlename"autocomplete=off > 
				</div>
			</div>
			<div class="col-md-4">
				<div class="mb-3">
